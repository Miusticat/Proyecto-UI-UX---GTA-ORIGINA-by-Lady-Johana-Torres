<template>
  <div class="hud" style="overflow-y: auto;background-color:transparent; height:100%; width:100%">
      ...
      <div class="info server" v-if="showhud">
        <div class="group-parent6">
          <div class="vector-parent">
            <img class="vector-icon6" alt="" src="../assets/images/hud/vector6.svg" />
            <div class="div17">{{("00000000" + money).slice(-8)}}$</div>
          </div>
          <div class="parent">
            <div class="div17">{{("00000000" + money).slice(-8)}}$</div>
            <img class="vector-icon7" alt="" src="../assets/images/hud/vector7.svg" />
          </div>
          <div class="group">
            <div class="div19">{{("00000000" + money).slice(-8)}}$</div>
            <img class="vector-icon8" alt="" src="../assets/images/hud/vector8.svg" />
          </div>
        </div>
        <div class="info player">
          ...
          <div class="group-parent2">
            <img class="group-child8" alt="" src="../assets/images/hud/sed.svg" />
            <div id="thirst" class="div12"></div>
          </div>
          <div class="group-parent3">
            <img class="group-child8" alt="" src="../assets/images/hud/hambre.svg" />
            <div id="hunger" class="div12"></div>
          </div>
          <div class="group-parent4">
            <img class="group-child8" alt="" src="../assets/images/hud/escudo.svg" />
            <div id="shield" class="div12"></div>
          </div>
          <div class="group-parent5">
            <img class="group-child8" alt="" src="../assets/images/hud/vida.svg" />
            <div id="health" class="div15"></div>
          </div>
          
          <!--------- END STATS BY MIUSTICAT  -->
        </div>
      </div>
    </div>
    ...
    <div class="death-container" v-if="showdeath">
      <div class="row justify-content-center centering">
        <div class="col-md-8 mt-3 animate__animated animate__fadeInUp">
          <div class="passport">
            <div class="container">
              <div class="text-center death-message">
                <p v-if="respawnTime > 0" class="respawn-text">¡Llamando a los servicios médicos! Tu personaje está incapacitado. Respawn disponible en {{getRespawnTime()}} minutos...</p>
                <p v-else class="respawn-text">¡Servicios médicos en camino! Si no llegan en 5 minutos, serás trasladado automáticamente al hospital.</p>
                <button v-if="respawnTime <= 0" @click="respawn()" type="button" class="btn btn-danger respawn-btn mt-3">Traslado al hospital</button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    // Function to update the HUD values
function updateHud() {
  document.getElementById('thirst').textContent = `%${hud.thirst}`;
  document.getElementById('hunger').textContent = `%${hud.hunger}`;
  document.getElementById('shield').textContent = `%${hud.shield}`;
  document.getElementById('health').textContent = `%${hud.health}`;
}

// Initial call to update the HUD on page load
updateHud();

// Function to simulate updating bars
...
// Using setTimeout to update bars after a delay
setTimeout(function() {
  updateBar(1, 30);  // Example value for health
  updateBar(2, 30);  // Example value for shield
  updateBar(3, 30);  // Example value for hunger
  updateBar(4, 30);  // Example value for thirst
}, 150);
    //Progressbars
    setTimeout(function () {
...
.div20 {
  ...
  left: 325.9px;
  font-weight: 500;
  text-shadow: 0px 2px 3px #000, 0px 4px 4px rgba(0, 0, 0, 0.25);
}
...
/* Inicio de estilos - DEATH PLAYER */
.death-container {
  background-color: rgba(0, 0, 0, 0.267); /* Fondo semi-transparente */
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
...
.group-child3 {
  ...
  top: 37.6px;
  left: 230px;
  box-shadow: 0px 0px 7.8px rgba(248, 11, 82, 0.66);
  border-radius: var(--br-8xs);
...
.n {
  ...
  top: 0px;
  left: 178px;
  font-size: var(--font-size-11xl-2);
  text-align: left;
}
.ne {
  position: absolute;
...
.div4 {
  ...
  left: 224px;
  color: var(--color-crimson-200);
  text-shadow: 0px 0px 7.8px rgba(248, 11, 82, 0.66);
}
.div5 {
  position: absolute;
  top: 15px;
...
.group-parent {
  ...
  top: 27px;
  left: 592px;
  filter: drop-shadow(0px 0px 1.4px #000);
  width: 736px;
  height: 72px;
